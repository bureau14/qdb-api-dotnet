...
namespace Quasardb.TimeSeries
{
    ...
    public sealed class QdbSymbolColumn : QdbColumn
    {
        ...
        public void Insert(QdbSymbolPointCollection points)
        {
            var error = qdb_api.qdb_ts_symbol_insert(Handle, Series.Alias, Name, points.Points.Buffer, points.Points.Count);
        ...
        {
            using (var pointCollection = new QdbSymbolPointCollection(points))
            {
        ...
        /// <param name="points">The points to insert</param>
        public void InsertTruncate(IEnumerable<QdbTimeInterval> intervals, QdbSymbolPointCollection points)
        {
            var ranges = new InteropableList<qdb_ts_range>(Helpers.GetCountOrDefault(intervals));
            foreach (var interval in intervals)
                ranges.Add(interval.ToNative());
            var error = qdb_api.qdb_ts_symbol_insert_truncate(
        ...
        /// <param name="points">The points to insert</param>
        public void InsertTruncate(QdbTimeInterval interval, QdbSymbolPointCollection points)
        {
        ...
        public Point First()
        {
            var result = _aggregator.SymbolAggregate(qdb_ts_aggregation_type.First);
            if (result.Count() == 0)
        ...
        {
            return _aggregator.SymbolAggregate(qdb_ts_aggregation_type.First, interval).AsPoint();
        }
        ...
        {
            return _aggregator.SymbolAggregate(qdb_ts_aggregation_type.First, intervals).AsPoint();
        }
        ...
        public Point Last()
        {
            var result = _aggregator.SymbolAggregate(qdb_ts_aggregation_type.Last);
            if (result.Count() == 0)
                throw new QdbEmptyColumnException(Series.Alias, Name);
            return result.AsPoint();
        }
        ...
        {
            return _aggregator.SymbolAggregate(qdb_ts_aggregation_type.Last, interval).AsPoint();
        }
        ...
        {
            return _aggregator.SymbolAggregate(qdb_ts_aggregation_type.Last, intervals).AsPoint();
        }
        ...
        {
            return _aggregator.SymbolAggregate(qdb_ts_aggregation_type.Count, interval).Count();
        }
        ...
        {
            return _aggregator.SymbolAggregate(qdb_ts_aggregation_type.Count, intervals).Count();
        }
