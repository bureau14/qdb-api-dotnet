...
namespace Quasardb.Tests.Entry.TimeSeries.Blob
{
    ...
    public class Erase
    {
        private QdbBlobPoint[] _points =
            {
                new QdbBlobPoint(new DateTime(2000, 01, 01), new byte[]{1}),
                new QdbBlobPoint(new DateTime(2000, 01, 02), new byte[]{2}),
            };

        public QdbBlobColumn CreateColumn(string alias = null, string name = null)
        {
            ...
            var col = ts.BlobColumns[colName];
            col.Insert(_points);
            return col;
        }
